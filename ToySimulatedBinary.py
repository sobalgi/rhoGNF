...
def load_data(name,
    ...
    elif name == "toy_sim_binary":
        return UCIdatasets.TOY_SIM_BINARY(n_samples=int(40000/.8), 
#                                   new=True, 
...
def train(dataset="toy_sim_binary", load=True, nb_step_dual=100, nb_steps=20, path="", l1=.1, nb_epoch=10000,
...
#     logger = utils.get_logger(logpath=os.path.join(path, 'logs'), filepath=os.path.abspath('/home/user/prj/rhoGNF/ToySimulatedBinary.py'))
...
import argparse
datasets = ["toy_sim_binary", "power", "gas", "bsds300", "miniboone", "hepmass", "digits", "proteins"]

parser = argparse.ArgumentParser(description='')
parser.add_argument("-load_config", default=None, type=str)
# General Parameters
parser.add_argument("-dataset", default='toy_sim_binary', choices=datasets, help="Which toy problem ?")
...
parser.add_argument("-p_U", default=0.4, type=float, help="DGP parameter")
parser.add_argument("-p_AU0", default=0.4, type=float, help="DGP parameter")
parser.add_argument("-p_AU1", default=0.6, type=float, help="DGP parameter")
parser.add_argument("-p_YA0U0", default=0.3, type=float, help="DGP parameter")
parser.add_argument("-p_YA0U1", default=0.4, type=float, help="DGP parameter")
parser.add_argument("-p_YA1U0", default=0.6, type=float, help="DGP parameter")
parser.add_argument("-p_YA1U1", default=0.7, type=float, help="DGP parameter")
parser.add_argument("-n_DGPs", default=20, type=int, help="Number of DGPs")
parser.add_argument("-n_dgp", default=0, type=int, help="DGP id")
...
path = "NeurIPS2022_rhoGNF/" + dir_name + "/" +  f'{args.n_DGPs:04d}_{args.n_dgp:04d}_' + now.strftime("%Y_%m_%d_%H_%M_%S") + '_' + machinename + '_rho_' +  f'{args.rho:.4f}' if args.folder == "" else args.folder
...
train(args.dataset, load=args.load, path=path, nb_step_dual=args.nb_steps_dual, l1=args.l1, nb_epoch=args.nb_epoch, nb_estop=args.nb_estop,
      ...
      cond_type=args.conditioner,  norm_type=args.normalizer,  n_mce_samples=args.n_mce_samples, mce_b_size=args.mce_b_size,
        ...
        p_AU0 = args.p_AU0,
        p_AU1 = args.p_AU1,
        p_YA0U0 = args.p_YA0U0,
        p_YA0U1 = args.p_YA0U1,
        p_YA1U0 = args.p_YA1U0,
        p_YA1U1 = args.p_YA1U1,
        n_DGPs = args.n_DGPs,
        n_dgp = args.n_dgp,
        rho = args.rho
#         p_U = p_U,
